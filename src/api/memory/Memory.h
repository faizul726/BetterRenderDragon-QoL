...
template <typename RTN = void, typename... Args>
constexpr auto virtualCall(uintptr_t self, uintptr_t off, Args&&... args) -> RTN {
    return (*(RTN(**)(uintptr_t, Args && ...))(*(uintptr_t*)self + off))(self, std::forward<Args>(args)...);
}
...
template <typename T>
std::optional<T> safeDAccess(void* ptr, intptr_t off) {
...
#define CC_CALL(iname, address, Ret, ...) ((Ret(*)(__VA_ARGS__))(memory::resolveSignature(address)))