...
void memory::WriteEx(uintptr_t fovAddr, const std::vector<uint16_t>& bytes, const std::string& patchName) {
    DWORD                 oldprotect;
...
void memory::WriteEx(memory::FuncPtr fovAddr, const std::vector<uint16_t>& bytes, const std::string& patchName) {
    auto address = reinterpret_cast<uintptr_t>(fovAddr);
    memory::WriteEx(address, bytes, patchName);
}

void memory::WriteEx(uintptr_t fovAddr, const std::string& byteStr, const std::string& patchName) {
    std::vector<uint16_t> bytes = StringToBytes(byteStr);
    memory::WriteEx(fovAddr, bytes, patchName);
}

void memory::WriteEx(memory::FuncPtr fovAddr, const std::string& byteStr, const std::string& patchName) {
    auto address = reinterpret_cast<uintptr_t>(fovAddr);
